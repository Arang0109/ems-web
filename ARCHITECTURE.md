...
| 레이어 | 개수 | 슬라이스 |
|--------|------|---------|
| pages | 9 그룹 / 라우트 22개 | `sign-in`, `dashboard`, `client`(하위 `client`·`contract`·`pollutant`·`measurement-method`), `equipment`, `schedule`, `staff`, `chat`, `admin`(하위 `member`·`document`), `platform`(하위 `tenant`·`pollutant-catalog`) |
| widgets | 25 | `sign-in`, `layouts`, `chat-room-list`, `chat-room`, `metrics`, `dashboard-stats`, `dashboard-alerts`, `client-table`, `workplace-table`, `stack-table`, `stack-list-table`, `stack-profile`, `contract-table`, `pollutant-table`, `pollutant-catalog-table`, `measurement-method-table`, `document-table`, `equipment-table`, `member-table`, `team-table`, `team-schedule-table`, `schedule-table`, `canceled-schedule-table`, `schedule-profile`, `tenant-table` |
| features | 52 | `sign-in`, `sign-out`, `send-chat-message`, `open-chat-room`, `hide-chat-room`, `download-chat-attachment`, `dashboard-summary`, `provision-tenant`, `record-inspection`, `download-document`, `add-document-version`, `delete-document-version`, `export-schedule-report`, `manage-schedule-lifecycle`, `save-schedule-sheets`, `save-schedule-analysis`, `fill-default-measurement-methods`, `register-*`(client·workplace·stack·contract·pollutant·pollutant-catalog·measurement-method·facility·prevention·stack-pollutant·document·equipment·member·schedule·team), `update-*`(client·contract·workplace·stack·stack-pollutant·facility·prevention·document·equipment·member·pollutant·pollutant-catalog·measurement-method·team·schedule-basic-info·schedule-client·schedule-equipments·schedule-item·schedule-items·schedule-stack) |
| entities | 18 | `auth`, `chat`, `client`, `workplace`, `stack`, `stack-pollutant`, `contract`, `dashboard`, `pollutant`, `pollutant-catalog`, `measurement-method`, `document`, `equipment`, `measurement-record`, `member`, `schedule`, `team`, `tenant` |
