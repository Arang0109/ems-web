// forked from design-sync lib/dts.mjs — ems-web 는 라이브러리가 아니라 앱이라
// 배포용 .d.ts 트리가 없다. 원본은 `<root>/**/*.d.ts` 만 ts-morph 프로젝트에
// 넣으므로 프로젝트가 사실상 비고, 모든 <Name>Props 가 `[key: string]: unknown`
// 스텁으로 떨어진다 (.d.ts 는 디자인 에이전트가 보는 유일한 API 계약이다).
//
// 이 포크가 바꾸는 것 (projectFor 한 곳뿐):
//   1) 엔트리가 없으면 `.design-sync/entry.ts`(그룹 배럴 재export)를 엔트리로 쓴다.
//   2) 그때 `src/{shared,components,lib}` 의 .ts/.tsx 를 프로젝트에 함께 넣는다.
//   3) `.design-sync/tsconfig.paths.json` 의 경로 별칭을 컴파일러 옵션에 준다
//      (없으면 `@shared/...` import 가 any 로 무너진다).
// 그러면 propsBodyFor 의 폴백(엔트리 export 의 첫 call signature 파라미터 타입)이
// 실제 Props 를 구조적으로 풀어낸다 — 이 저장소는 인터페이스 이름이 `Props` 라서
// <Name>Props 이름 조회로는 절대 안 잡힌다.
//
...
function projectFor(pkgDir, typesRoot) {
  ...
  let entry = join(pkgDir, (pubEntry && existsSync(join(pkgDir, pubEntry)) ? pubEntry : null) || pj.types || pj.typings || 'index.d.ts');
  // ── fork: 소스 저장소 모드 ────────────────────────────────────────────────
  const dsDir = join(pkgDir, '.design-sync');
  const srcEntry = join(dsDir, 'entry.ts');
  const srcMode = !existsSync(entry) && existsSync(srcEntry);
  if (srcMode) entry = srcEntry;
  let paths;
  if (srcMode) {
    try { ({ paths } = JSON.parse(readFileSync(join(dsDir, 'tsconfig.paths.json'), 'utf8')).compilerOptions); }
    catch { /* 별칭 없이도 상대 import 는 풀린다 */ }
  }
  ...
  // ── fork: .d.ts 대신 실제 소스를 넣는다 (번들 그래프가 닿는 범위로 제한) ──
  if (srcMode) {
    const g = (p) => join(pkgDir, p).split('\\').join('/');
    project.addSourceFilesAtPaths([
      g('src/shared/**/*.{ts,tsx}'),
      g('src/components/**/*.{ts,tsx}'),
      g('src/lib/**/*.{ts,tsx}'),
      `!${g('src/**/*.test.{ts,tsx}')}`,
    ]);
    project.addSourceFileAtPath(srcEntry);
  }
  console.error(`  [DTS] parsed ${project.getSourceFiles().length} ${srcMode ? 'source' : '.d.ts'} files from ${root}`);
