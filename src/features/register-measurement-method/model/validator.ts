...
/**
 * 서버 규칙과 같다 — 이름·채취 단위는 필수, 통칭명은 `MERGED` 에서만 필수(그 외에는 mapper 가 버린다),
 * 채취시간은 0 이상의 정수.
 */
export const validateMeasurementMethodRegisterFields = (form: MeasurementMethodRegisterForm) => {
  const errors: Partial<Record<keyof MeasurementMethodRegisterForm, string>> = {};
