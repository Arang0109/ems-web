...
export const validateEquipmentSpec = (type: EquipType, spec: EquipmentSpecForm): string | undefined => {
  switch (type) {
    case 'PARTICLE_SAMPLER':
      if (![spec.totalVolume, spec.orificeDp, spec.yd].every(isNonNegativeNumber)) {
        return `${EQUIP_SPEC_FIELD_LABEL.totalVolume}·${EQUIP_SPEC_FIELD_LABEL.orificeDp}·${EQUIP_SPEC_FIELD_LABEL.yd} 값을 모두 입력해주세요.`;
      }
      return undefined;
    case 'GAS_SAMPLER':
    case 'OTHER':
      if (!isNonNegativeNumber(spec.totalVolume)) return `${EQUIP_SPEC_FIELD_LABEL.totalVolume} 값을 입력해주세요.`;
      return undefined;
    case 'PITOT_TUBE':
      ...
      if (!spec.coefficients.every((c) => isPositiveNumber(c.coefficient) && isPositiveNumber(c.velocity))) {
    ...
    case 'NOZZLE':
      ...
      if (!spec.diameters.every((d) => isPositiveNumber(d.diameter))) {
...
export const validateInspectionForms = (inspections: InspectionItemForm[]): string | undefined => {
  const invalid = inspections.find(
    (item) => item.enabled && item.cycleMonths.trim() !== '' && !isPositiveNumber(item.cycleMonths)
  );
