...
export const validateStackFields = (form: StackUpdateForm) => {
  const errors: Partial<Record<keyof StackUpdateForm, string>> = {};

  if (!form.name.trim()) {
    errors.name = "측정시설명을 입력해주세요.";
  }

  // 기준산소농도는 선택 항목이라 빈 값은 통과시킨다(= 서버가 기존 값을 유지).
