...
//
// **분류가 성적서(REPORT_TEMPLATE)가 아니라 채취기록부(SAMPLING_RECORD_TEMPLATE)인 것은 의도다.**
// 성적서 탭의 다운로드가 당분간 `exportSamplingRecords` 를 타므로(useExportReport 참고)
// 양식도 그 API 의 jxls 계약(단일 시트 바인딩)을 따르는 쪽이어야 한다.
// 성적서 export 로 되돌릴 때 이 분류도 함께 REPORT_TEMPLATE 로 되돌린다.
...
export type ReportTemplate = ReturnType<typeof useReportTemplate>;
