...
export const TemplateCheckResult = ({ result }: Props) => {
  if (result.valid) {
    return (
      <p className="mt-3 flex items-center gap-2 text-caption text-muted-ink">
        ...
        {result.issues.map((issue, index) => {
          ...
          return (
            <li key={`${location}-${issue.name}-${index}`} className="flex flex-col gap-0.5 sm:flex-row sm:gap-2">
              <span className="shrink-0 font-mono text-muted-ink">{location}</span>
              <span className="shrink-0 font-medium">{title}</span>
              <span className="break-all text-muted-ink">{detail}</span>
            </li>
          );
        })}
      </ul>
