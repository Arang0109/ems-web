...
const METHOD: Record<string, MeasurementMethodSnapshot> = {
  DUST:                { methodId: 1, name: "먼지",     sampleGrouping: "NONE",     mergedSampleName: null,     samplingMinutes: null, suctionFlowRate: null },
  HEAVY_METAL:         { methodId: 2, name: "중금속",   sampleGrouping: "NONE",     mergedSampleName: null,     samplingMinutes: null, suctionFlowRate: null },
  MERCURY:             { methodId: 3, name: "수은",     sampleGrouping: "NONE",     mergedSampleName: null,     samplingMinutes: null, suctionFlowRate: null },
  FIELD_MEASUREMENT:   { methodId: 4, name: "현장측정", sampleGrouping: "NONE",     mergedSampleName: null,     samplingMinutes: null, suctionFlowRate: null },
  ABSORPTION_SOLUTION: { methodId: 5, name: "흡수액",   sampleGrouping: "PER_ITEM", mergedSampleName: null,     samplingMinutes: 40, suctionFlowRate: null },
  ADSORPTION_TUBE:     { methodId: 6, name: "흡착관",   sampleGrouping: "MERGED",   mergedSampleName: "VOCs-T", samplingMinutes: 30, suctionFlowRate: null },
  TEDLAR_BAG:          { methodId: 7, name: "테드라백", sampleGrouping: "PER_ITEM", mergedSampleName: null,     samplingMinutes: null, suctionFlowRate: null },
  CARTRIDGE:           { methodId: 8, name: "카트리지", sampleGrouping: "MERGED",   mergedSampleName: "VOCs",   samplingMinutes: 30, suctionFlowRate: null },
};
...
describe("buildGasSampleGroups", () => {
  it("흡착관 항목을 VOCs-T 한 행으로, 카트리지 항목을 VOCs 한 행으로 묶는다", () => {
    ...
    expect(groups).toEqual([
      { key: "method:6", sampleName: "VOCs-T", pollutantIds: [1, 2], particulateSource: null, suctionFlowRate: null },
      { key: "method:8", sampleName: "VOCs", pollutantIds: [3, 4], particulateSource: null, suctionFlowRate: null },
    ]);
  });
  ...
  it("입자상 항목이라도 phase 가 아니라 붙인 측정방법의 채취 단위를 따른다 (PAH)", () => {
    const particulateCartridge: MeasurementMethodSnapshot = {
      methodId: 42, name: "카트리지(입자상)", sampleGrouping: "NONE", mergedSampleName: null, samplingMinutes: null, suctionFlowRate: null,
    };
    const groups = buildGasSampleGroups([
    ...
    expect(groups).toEqual([
      { key: "method:8", sampleName: "VOCs", pollutantIds: [9], particulateSource: null, suctionFlowRate: null },
    ]);
  });
  ...
  it("입자상 항목을 통칭 채취 방법에 붙이면 회사 데이터대로 뭉친다", () => {
    ...
    expect(groups).toEqual([{ key: "method:8", sampleName: "VOCs", pollutantIds: [7, 9], particulateSource: null, suctionFlowRate: null }]);
  });
  ...
  it("원장 연결키가 없는 구 문서는 측정방법 이름으로 묶는다", () => {
    ...
    expect(groups).toEqual([
      { key: "method-name:카트리지", sampleName: "VOCs", pollutantIds: [3, 4], particulateSource: null, suctionFlowRate: null },
    ]);
  });
  ...
  it("고객사가 새로 만든 통칭 측정방법도 그 통칭명으로 묶는다", () => {
    const custom: MeasurementMethodSnapshot = {
      methodId: 42, name: "흡착관(저농도)", sampleGrouping: "MERGED", mergedSampleName: "VOCs-L", samplingMinutes: 60, suctionFlowRate: null,
    };
    const groups = buildGasSampleGroups([
    ...
    expect(groups).toEqual([{ key: "method:42", sampleName: "VOCs-L", pollutantIds: [1, 2], particulateSource: null, suctionFlowRate: null }]);
  });
...
describe("표준 흡인유량 시드", () => {
  it("항목의 유효 흡인유량이 행의 흡인유량 칸에 채워지고, 통칭 행은 첫 항목의 값을 쓴다", () => {
    const groups = buildGasSampleGroups([
      { ...item(1, "이산화황", "ABSORPTION_SOLUTION", "GAS"), suctionFlowRate: 1.5 },
      { ...item(2, "벤젠", "ADSORPTION_TUBE", "GAS"), suctionFlowRate: 0.2 },
      { ...item(3, "톨루엔", "ADSORPTION_TUBE", "GAS"), suctionFlowRate: 0.2 },
    ]);

    expect(groups.map((g) => g.suctionFlowRate)).toEqual([1.5, 0.2]);
    expect(groups.map((g) => toSampleForm(g).suctionQuantity)).toEqual(["1.5", "0.2"]);
  });

  it("흡인유량이 정해지지 않은 항목의 행은 빈 칸으로 시작한다", () => {
    const [group] = buildGasSampleGroups([item(1, "암모니아", "ABSORPTION_SOLUTION", "GAS")]);

    expect(toSampleForm(group).suctionQuantity).toBe("");
  });
});

describe("hydrateSheets — 빈 흡인유량 칸 채우기", () => {
  const groups = buildGasSampleGroups([
    { ...item(1, "이산화황", "ABSORPTION_SOLUTION", "GAS"), suctionFlowRate: 1.5 },
    item(2, "암모니아", "ABSORPTION_SOLUTION", "GAS"),   // 표준값 없음
  ]);

  it("저장된 행이라도 흡인유량이 비어 있으면 항목 표준값으로 채운다", () => {
    const [sheet] = hydrateSheets([sheetWith([sample([1], "이산화황"), sample([2], "암모니아")])], groups);

    expect(sheet.samples.map((s) => s.suctionQuantity)).toEqual(["1.5", ""]);
  });

  it("이미 적힌 흡인유량은 실측일 수 있어 건드리지 않는다", () => {
    const measured = { ...sample([1], "이산화황"), suctionQuantity: "1.2" };
    const [sheet] = hydrateSheets([sheetWith([measured])], groups);

    expect(sheet.samples[0].suctionQuantity).toBe("1.2");
  });

  it("채울 칸이 없으면 원본 배열을 그대로 돌려준다", () => {
    const sheets = [sheetWith([{ ...sample([1], "이산화황"), suctionQuantity: "1.2" }, sample([2], "암모니아")])];

    expect(hydrateSheets(sheets, groups)).toBe(sheets);
  });

  it("항목 링크가 없는 수동 행은 채우지 않는다", () => {
    const [sheet] = hydrateSheets([sheetWith([sample([1], "이산화황"), sample([], "직접 추가")])], groups);

    expect(sheet.samples[1].suctionQuantity).toBe("");
  });
});
