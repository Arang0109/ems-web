// gaseous/ — 가스상 시료 행. 측정항목이 어느 행(병)에 담기는가(gaseous-rows)와 행이 담은 항목에서 유도되는 규칙
...
export type GasSampleGroup = {
  ...
  /**
   * 이 병의 표준 흡인유량(L/min) — 새 행의 흡인유량 칸을 이 값으로 채워 시작한다. 항목의 유효값(방법 기본값 +
   * 항목 오버라이드)이며, 통칭 행은 한 펌프로 잡으므로 첫 항목의 값이 곧 병의 값이다. 미지정이면 null.
   */
  ...
  for (const item of items) {
    ...
    if (item.method.sampleGrouping !== "MERGED" || item.method.mergedSampleName === null) {
      groups.push({
        ...
        sampleName: item.nameEn ? item.nameEn : item.nameKr,
        pollutantIds: [item.pollutantId],
        particulateSource: particulateSourceOf(item.mode),
        suctionFlowRate: item.suctionFlowRate,
      });
      continue;
    }
...
/**
 * 그룹을 시료 행으로 편다 — 항목명·링크와 표준 흡인유량만 채우고 실측값은 사용자가 적는다.
 * 흡인유량은 계획값이 있는 칸이라 시드한다(정유량 펌프는 정해 둔 유량으로 잡는다). 사용자가 고칠 수 있고,
 * 등속흡인 행은 어차피 잠긴 파생 칸이라 이 값이 보이지 않는다.
 */
export const toSampleForm = (group: GasSampleGroup): SampleForm => ({
  ...
  suctionQuantity: toFormValue(group.suctionFlowRate),
});

/**
 * 서버에서 받은 기록지에 아직 적히지 않은 그룹을 채워 넣고, 행의 빈 흡인유량 칸을 표준값으로 채운다.
 *
 * 행 추가는 **가스상 표가 비어 있는 기록지에만** 한다. 이미 행이 있는 기록지는 사용자가 구성을 정해 둔
 * 것이므로 행을 건드리지 않고, 화면에서 "미배정 N건" 으로 알리기만 한다. 다만 그 행들의 빈 흡인유량 칸은
 * 채운다({@link seedSuctionFlowRates}) — 행 구성이 아니라 칸의 기본값이라 사용자 판단을 되돌리는 것이 아니다.
 *
...
export const hydrateSheets = (
  sheets: SheetForm[],
  groups: GasSampleGroup[],
): SheetForm[] => seedSuctionFlowRates(fillUnassignedRows(sheets, groups), groups);

/** 빈 가스상 표를 미배정 그룹으로 채운다 — 위 {@link hydrateSheets} 의 첫 규칙 */
const fillUnassignedRows = (sheets: SheetForm[], groups: GasSampleGroup[]): SheetForm[] => {
...
/**
 * 이미 있는 행의 **빈** 흡인유량 칸을 항목의 표준 흡인유량으로 채운다.
 *
 * 새 행은 {@link toSampleForm} 이 시드하지만, 저장된 적 있는 기록지의 행은 표준값이 나중에 정해졌거나
 * (측정방법·측정물질에 흡인유량이 없던 때 만든 행) 사용자가 아직 적지 않은 채라 비어 있다. 비어 있는 칸은
 * "미입력"이지 "0"이 아니므로 채워도 잃는 값이 없고, 적힌 값은 실측일 수 있어 건드리지 않는다.
 * 규칙이 결정적이라 기준선·원격 병합도 같은 값을 보며, 바뀐 행이 없으면 원본 배열을 그대로 돌려준다.
 */
const seedSuctionFlowRates = (sheets: SheetForm[], groups: GasSampleGroup[]): SheetForm[] => {
  const rateByPollutantId = new Map(
    groups.flatMap((group) =>
      group.suctionFlowRate === null ? [] : group.pollutantIds.map((id) => [id, group.suctionFlowRate] as const)),
  );
  if (rateByPollutantId.size === 0) return sheets;

  let changed = false;
  const seeded = sheets.map((sheet) => {
    let sheetChanged = false;
    const samples = sheet.samples.map((sample) => {
      if (sample.suctionQuantity.trim() !== "") return sample;
      const rate = sample.pollutantIds.map((id) => rateByPollutantId.get(id)).find((r) => r != null);
      if (rate == null) return sample;
      sheetChanged = true;
      return { ...sample, suctionQuantity: toFormValue(rate) };
    });
