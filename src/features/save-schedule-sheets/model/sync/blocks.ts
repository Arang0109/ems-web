// sync/ — 동시편집. 시트를 블록으로 갈라(blocks) 내가 바꾼 것만 저장하고(conflict: 기준선·409 복구),
// 다른 사용자의 저장을 내 편집을 지키며 병합한다(remote-sync). 훅이 아니라 순수 함수라 전부 테스트가 있다.
...
/** 지정한 블록 하나만 `source` 값으로 바꾼 시트를 반환한다. 병합·기록지 간 복사가 같은 경로를 쓴다. */
export const applyBlock = (target: SheetForm, source: SheetForm, block: SheetBlock): SheetForm => {
  switch (block) {
