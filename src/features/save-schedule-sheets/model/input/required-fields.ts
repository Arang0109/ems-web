// input/ — 입력 칸의 규칙. 어떤 칸이 있고(경로), 무엇이 필수이며(required-fields), 얼마나 채웠고(section-progress),
// 저장을 막을 값은 무엇인지(validator), 지점 간에 어떻게 전파되는지(point-chain), 칸의 설명은 무엇인지(field-hints).
// 사용자가 **적는 쪽**의 규칙만 둔다 — 입력에서 계산되는 값은 derived/ 에 있다.

import type { SheetSectionId } from "../sections";
import type {
  GasColumnKey, MoistureForm, ParticleForm, SampleFieldKey, SampleForm, SamplingPointForm,
...
export const fieldPath = {
  ...
  exhaustReading: (key: GasColumnKey, index: number): SheetFieldPath => `exhaustGas.${key}.${index}`,
...
const GAS_COLUMNS: GasColumnKey[] = ["o2", "co2", "co", "nox", "sox"];
  ...
  switch (head) {
    ...
    case "exhaustGas":
      return b === undefined
        ...
        : sheet.exhaustGas[a as GasColumnKey]?.[Number(b)] ?? "";
    case "points":
...
/** 칸에 값이 있는가 — 공백만 있는 칸은 비어 있다. 필수 판정·진행도·불러온 값 추적이 전부 이 하나를 본다 */
...
const BASE_GAS_COLUMNS: GasColumnKey[] = ["o2", "co2", "co"];
...
export const getRequiredFields = (
  ...
  const readings = (column: GasColumnKey): SheetFieldPath[] =>
