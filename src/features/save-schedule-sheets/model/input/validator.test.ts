...
describe("calcMoistureWeightGain", () => {
  it("후 − 전 이다", () => {
    expect(calcMoistureWeightGain(moisture("12.20", "12.55"))).toBeCloseTo(0.35, 10);
  });

  it("전·후 중 하나라도 비어 있으면 null 이다", () => {
    expect(calcMoistureWeightGain(moisture("12.20", ""))).toBeNull();
    expect(calcMoistureWeightGain(moisture("", "12.55"))).toBeNull();
    expect(calcMoistureWeightGain(moisture("", ""))).toBeNull();
  });
});
...
describe("checkMoistureWeightGain", () => {
  ...
  it("경계값은 허용한다 — 부동소수 뺄셈 오차에 밀려나지 않는다", () => {
    ...
    expect(calcMoistureWeightGain(moisture("12.25", "12.35"))).toBeLessThan(
    ...
    expect(calcMoistureWeightGain(moisture("1.2", "2.2"))).toBeGreaterThan(
