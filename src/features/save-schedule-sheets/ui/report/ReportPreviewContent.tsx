...
export const ReportPreviewContent = ({ sheet, preview, snapshot, basicInfoForm, externals }: Props) => {
  ...
  const suctionVelocity = toNumberOrNull(sheet.moisture.suctionVelocity);
  const moistureSamplingTime =
    moistureVolume != null && suctionVelocity != null && suctionVelocity !== 0
      ? moistureVolume / suctionVelocity : null;
        ...
        <table className="border-collapse text-[9px] leading-tight w-full" style={{ minWidth: "800px" }}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <VCell colSpan={5}>{client.name}</VCell>
              <VCell rowSpan={5} colSpan={6}>
                <div className="mb-1">굴뚝높이: {stack.height ?? "-"} m</div>
            ...
            <tr>
              ...
              <VCell colSpan={1}>{quantity?.Cp ?? "-"}</VCell>
            </tr>
            <tr>
              <TableLabelCell colSpan={4}>측 정 일</TableLabelCell>
              <VCell colSpan={5}>{basicInfo.sampledAt}</VCell>
            ...
            <tr>
              ...
              <VCell colSpan={1}>{samplerSpec?.yd ?? "-"}</VCell>
            </tr>
            ...
            <tr>
              ...
              <TableLabelCell colSpan={5}>흡인노즐 (cm)</TableLabelCell>
              <VCell colSpan={3}>{sheet.particle.nozzleSize || "-"}</VCell>
            </tr>
            ...
            <tr>
              ...
              <VCell>{startVolume == null ? "0.00" : (startVolume * 1000).toFixed(2)}</VCell>
            </tr>
            <tr>
            ...
            {Array.from({ length: Math.max(5, points.length) }, (_, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <VCell>{after == null ? "" : (after * 1000).toFixed(2)}</VCell>
                  <VCell>{before != null && after != null ? ((after - before) * 1000).toFixed(2) : ""}</VCell>
                </tr>
              );
            })}
            ...
            <tr>
              ...
              <VCell>{lastAfterVm == null ? "" : (lastAfterVm * 1000).toFixed(2)}</VCell>
            </tr>
            ...
            <tr>
              ...
              <VCell colSpan={3}>{fmt(moistureSamplingTime, 1)}</VCell>
            ...
            <tr>
              <VCell colSpan={3}>{sheet.moisture.suctionVelocity || "-"}</VCell>
              ...
              <VCell colSpan={3}>{sheet.moisture.dryGasVolumeBefore || "-"}</VCell>
              <VCell colSpan={3}>{sheet.moisture.dryGasVolumeAfter || "-"}</VCell>
            ...
            {gasRows.map(({ sample, item }, i) => (
              <tr key={i}>
                <VCell colSpan={3}>{item?.nameKr ?? "-"}</VCell>
                <VCell colSpan={4}>{sample ? `${sample.startTime || "--:--"} ~ ${sample.endTime || "--:--"}` : ""}</VCell>
