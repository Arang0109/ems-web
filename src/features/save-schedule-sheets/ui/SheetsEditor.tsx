...
import { Calculator, Download, FileCheck, History, Plus, Save, X } from "lucide-react";
...
import { getAssignedPollutants } from "../model/measured-pollutants";
...
export const SheetsEditor = ({ scheduleId, snapshot, editable, externals, onSaved }: Props) => {
  ...
  } = useLoadPreviousSheet({ scheduleId, activeSheet, updateActiveSheet });
    ...
    <div className="space-y-4">
      ...
      {(activeSheet || isDirty) && (
        ...
        <StickyActionBar className="rounded-t-panel flex items-center justify-center">
          {timeline && <SamplingTimelinePopover timeline={timeline} isMobile={isMobile} />}
          ...
          >
            <Calculator size={19} />{!isMobile && `계산값`}
          </Button>

          <Button type="button" variant="soft" onClick={() => setPreviewOpen(true)}>
            <FileCheck size={19} />{!isMobile && `미리보기`}
          </Button>
          ...
          >
            <Download size={19} />{!isMobile && `다운로드`}
          </Button>
          {editable && (
            <>
