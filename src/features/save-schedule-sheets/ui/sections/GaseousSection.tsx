 ...
 * 행 순서 조작.
 *
 * 드래그 대신 버튼인 이유는 표가 12열 가로 스크롤이라 끌어 옮길 자리가 없기 때문이다.
 ...
 *
 * 행=항목, 열=입력 칸인 표 하나를 **모든 폭에서** 쓴다 — 모바일도 가로 스크롤이며, 항목명 열은 왼쪽에 고정된다.
 * 카드+모달(값이 안 보여 항목 간 대조 불가)과 칸을 시점별로 묶은 표 여러 개(대조는 되지만 한 항목을
...
export const GaseousSection = ({
  ...
  const columns: InputTableColumn<SampleForm>[] = [
    ...
    ...GAS_FIELDS.map((f): InputTableColumn<SampleForm> => ({
      kind: "input",
      // 항목명은 행의 이름이다 — 가로 스크롤에서 왼쪽에 남아 어느 행의 값인지 알려 준다.
      // No. 는 고정하지 않는다 — 같이 고정하면 좁은 화면에서 값 열이 한 칸 반밖에 안 보인다.
      sticky: f.field === "sampleName",
      header: (
        <>
