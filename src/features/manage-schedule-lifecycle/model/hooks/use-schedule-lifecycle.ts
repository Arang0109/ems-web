...
import { useReasonDialog, type ReasonDialogState } from "./use-reason-dialog";
...
interface Result {
  ...
  handleComplete: () => void;
  handleDelete: () => void;
  cancelDialog: ReasonDialogState;
  reopenDialog: ReasonDialogState;
}

/**
 * 측정계획 생애주기 확정 — 완료·취소·삭제·재개방.
 *
 * 진행 축(완료·취소)과 가시성 축(삭제)은 서로 독립이다.
 * - **취소**: 실재한 계획이 무산된 것. 사유가 이력에 기록되고 취소 목록으로 따로 관리된다.
 * - **삭제**: 목록에서 감추는 것. 실측 데이터가 없는 '측정예정'과 '취소'에서만 가능하며 복구할 수 있다.
 *
...
export const useScheduleLifecycle = ({ scheduleId, status, onSuccess }: Params): Result => {
  ...
  // 삭제 후에는 상세 화면의 대상이 사라지므로, 감춰진 계획이 모이는 목록으로 돌려보낸다.
  ...
  const cancelDialog = useReasonDialog({
    onSubmit: async (reason) => {
      try {
        await cancelSchedule(scheduleId, reason);
  ...
  const reopenDialog = useReasonDialog({
    onSubmit: async (reason) => {
      try {
        await reopenSchedule(scheduleId, reason);
