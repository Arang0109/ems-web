...
/**
 * 성적서 탭 하단의 성적서 작성 완료 확정 카드.
 *
 * 헤더의 {@link ScheduleLifecycleActions} 와 같은 조작이지만, 성적서 항목 순서를 정하고 문서를 내려받는
 * 자리가 곧 성적서 작성이 끝나는 자리라 여기서 확정하는 것이 실제 업무 순서다. 확정 조건(분석값입력중)과
 * 확정의 결과(편집 잠금)를 함께 설명해야 하므로 헤더 버튼과 달리 안내문을 동반한다 —
 * 버튼이 아직 없는 이유(시료접수일 미입력)도 이 안내문이 말해 준다.
 */
...
const guideOf = (status: ScheduleStatus | null): string => {
  switch (status) {
    ...
    default:
      return "실험·분석 탭의 '분석 진행'에서 시료접수일자를 저장하면 분석값입력중으로 전환되고, 그때 확정 버튼이 나타납니다.";
  }
};
