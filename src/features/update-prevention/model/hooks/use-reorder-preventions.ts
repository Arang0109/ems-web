...
import { useReorderPreventionsAction } from '@entities/stack';
 ...
 */
export const useReorderPreventions = ({ stackId, preventions, onRefetch }: Props) => {
  const { reorderPreventions, isLoading } = useReorderPreventionsAction();

  // 서버 목록 위에 얹는 낙관적 순서. null 이면 서버 목록을 그대로 쓴다.
  const [optimistic, setOptimistic] = useState<Prevention[] | null>(null);
  ...
  const [syncedPreventions, setSyncedPreventions] = useState(preventions);
  if (syncedPreventions !== preventions) {
    setSyncedPreventions(preventions);
    setOptimistic(null);
  }
  ...
  const items = optimistic ?? preventions;
  ...
  const handleReorder = async (from: number, to: number) => {
    ...
    try {
      await reorderPreventions(stackId, next.map((prevention) => prevention.id));
    } catch (err) {
