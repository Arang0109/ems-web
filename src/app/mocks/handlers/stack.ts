...
export const stackHandlers = [
  http.get(`${BASE_URL}/stacks/:stackId`, ({ params }) => {
    ...
    if (!detail) {
      return fail('측정시설을 찾을 수 없습니다.', { status: 404 });
    }
    return ok(detail, '측정시설 상세 조회 성공');
  }),
  ...
  http.get(`${BASE_URL}/stacks`, ({ request }) => {
    ...
    return ok(stacks, '측정시설 목록 조회 성공');
  }),

  http.post(`${BASE_URL}/stacks`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Date.now(), ...body }, '측정시설 등록 성공', { status: 201 });
  }),

  http.put(`${BASE_URL}/stacks/:stackId`, async ({ params, request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Number(params.stackId), ...body }, '측정시설 수정 성공');
  }),

  // 방지시설 (preventions)
  http.post(`${BASE_URL}/preventions`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Date.now(), ...body }, '방지시설 등록 성공', { status: 201 });
  }),
  ...
  http.put(`${BASE_URL}/preventions/order`, async ({ request }) => {
    ...
    if (!detail) {
      return fail('측정지점을 찾을 수 없습니다.', { status: 404 });
    }

    const ordered = applyOrder(detail.preventions, orderedIds);
    if (!ordered) {
      return fail('시설 목록이 변경되었습니다. 새로고침 후 다시 시도해 주세요.', { status: 400 });
    }

    detail.preventions = ordered;
    return ok(null, '방지시설 순서 변경 성공');
  }),

  http.put(`${BASE_URL}/preventions/:preventionId`, async ({ params, request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Number(params.preventionId), ...body }, '방지시설 수정 성공');
  }),

  http.delete(`${BASE_URL}/preventions/:preventionId`, ({ params }) => {
    return ok(null, `${params.preventionId} 방지시설 삭제 완료`);
  }),

  // 연료시설 (facilities)
  http.post(`${BASE_URL}/facilities`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Date.now(), ...body }, '연료시설 등록 성공', { status: 201 });
  }),
  ...
  http.put(`${BASE_URL}/facilities/order`, async ({ request }) => {
    ...
    return ok(null, '배출시설 순서 변경 성공');
  }),

  http.put(`${BASE_URL}/facilities/:facilityId`, async ({ params, request }) => {
    const body = await request.json() as Record<string, unknown>;
    return ok({ id: Number(params.facilityId), ...body }, '연료시설 수정 성공');
  }),

  http.delete(`${BASE_URL}/facilities/:facilityId`, ({ params }) => {
    return ok(null, `${params.facilityId} 연료시설 삭제 완료`);
  }),
];
