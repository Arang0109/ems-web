...
import { INSPECTION_TYPE, type InspectionType } from '@shared/model';
...
const disabled = (type: InspectionType): MockInspectionItem => ({
...
const normalize = (items: MockInspectionItem[] | undefined): MockInspectionItem[] =>
  INSPECTION_TYPE.map((type) => items?.find((i) => i.type === type) ?? disabled(type));
...
export const equipmentHandlers = [
  ...
  http.post(`${BASE_URL}/equipments/:equipmentId/inspections`, async ({ params, request }) => {
    ...
    if (!target) {
      return fail('존재하지 않는 장비입니다.', { status: 404 });
    }

    const item = normalize(target.inspections).find((i) => i.type === body.type);
    if (!item?.enabled) {
      return fail('이 장비의 검사 대상이 아닌 검사 종류입니다.', { status: 400 });
    }
    ...
    return ok(created, '검사 이력 등록 성공', { status: 201 });
  }),
  ...
  http.patch(`${BASE_URL}/equipments/:equipmentId/status`, async ({ params, request }) => {
    ...
    if (!target) {
      return fail('장비를 찾을 수 없습니다.', { status: 404 });
    }
    target.status = body.status;
    target.modifiedAt = new Date().toISOString().slice(0, 19);
    return ok(toResponse(target), '장비 상태 변경 성공');
  }),
  ...
  http.get(`${BASE_URL}/equipments/:equipmentId`, ({ params }) => {
    ...
    return ok(toResponse(target), '장비 조회 성공');
  }),
  ...
  http.put(`${BASE_URL}/equipments/:equipmentId`, async ({ params, request }) => {
    ...
    return ok(toResponse(target), '장비 수정 성공');
  }),
  ...
  http.delete(`${BASE_URL}/equipments/:equipmentId`, ({ params }) => {
    ...
    return ok(null, `${params.equipmentId} 삭제 완료`);
  }),
  ...
  http.post(`${BASE_URL}/equipments`, async ({ request }) => {
    ...
    return ok(toResponse(created), '장비 등록 성공', { status: 201 });
  }),
];
