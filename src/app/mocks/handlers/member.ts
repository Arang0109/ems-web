...
export const roleHandlers = [
  http.get(`${BASE_URL}/roles`, () => {
    return ok(roleList, '역할 목록 조회 성공');
  }),
];
...
export const memberHandlers = [
  ...
  http.get(`${BASE_URL}/admin/members/:memberId`, ({ params }) => {
    ...
    if (!member) {
      return fail('회원을 찾을 수 없습니다.', { status: 404 });
    }
    return ok(member, '회원 조회 성공');
  }),

  http.get(`${BASE_URL}/admin/members`, () => {
    return ok(memberList, '회원 목록 조회 성공');
  }),
  ...
  http.delete(`${BASE_URL}/admin/members/:memberId`, ({ params }) => {
    return ok(null, `${params.memberId} 삭제 완료`);
  }),
];
