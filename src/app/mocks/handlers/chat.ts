...
export const chatHandlers = [
  ...
  http.get(`${BASE_URL}/chat/rooms/:roomId/messages/:messageId/attachment`, ({ params }) => {
    ...
    if (!target) return fail('존재하지 않는 메시지입니다.', { status: 404 });
    if (!target.attachment) return fail('첨부 파일이 없는 메시지입니다.', { status: 404 });
  ...
  http.post(`${BASE_URL}/chat/rooms/:roomId/messages/attachments`, async ({ params, request }) => {
    ...
    if (!rooms.some((r) => r.roomId === roomId)) return fail('존재하지 않는 대화방입니다.', { status: 404 });

    const form = await request.formData();
    const file = form.get('file');
    if (!(file instanceof File)) return fail('내용이나 첨부 파일 중 하나는 있어야 합니다.', { status: 400 });
    if (file.size > 10 * 1024 * 1024) return fail('첨부 파일은 10MB를 넘을 수 없습니다.', { status: 413 });
  ...
  http.post(`${BASE_URL}/chat/rooms/:roomId/messages`, async ({ params, request }) => {
    ...
    if (!body.content?.trim()) return fail('내용이나 첨부 파일 중 하나는 있어야 합니다.', { status: 400 });
  ...
  http.post(`${BASE_URL}/chat/rooms/:roomId/read`, async ({ params, request }) => {
    ...
    if (!room) return fail('존재하지 않는 대화방입니다.', { status: 404 });
    ...
    if (!OBJECT_ID_PATTERN.test(body.lastReadMessageId ?? '')) {
      return fail('메시지 id 형식이 올바르지 않습니다.', { status: 400 });
    }
  ...
  http.post(`${BASE_URL}/chat/rooms`, async ({ request }) => {
    ...
    if (body.counterpartId === MY_USER_ID) return fail('자기 자신과는 대화할 수 없습니다.', { status: 400 });
    if (!memberOf(body.counterpartId)) return fail('존재하지 않는 사용자입니다.', { status: 404 });
