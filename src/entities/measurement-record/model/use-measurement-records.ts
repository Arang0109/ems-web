...
import { measurementRecordKeys } from "./query-keys";
import type { MeasurementRecord } from "./types";

/**
 * 측정지점의 측정 이력을 <b>전체 기간</b>으로 조회한다.
 *
...
export const useMeasurementRecords = (stackId: number | null) => {
  const result = useEntityQuery<MeasurementRecord[]>({
    queryKey: measurementRecordKeys.list(stackId as number),
    queryFn: async () =>
      toMeasurementRecords(
        unwrapMessage(await measurementRecordApi.getMeasurementRecords(stackId as number)) ?? [],
      ),
    initialData: [],
  ...
  // (`enabled: false` 면 react-query 의 isLoading 은 false 라 이 보정이 계속 필요하다.)
