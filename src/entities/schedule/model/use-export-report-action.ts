...
export const useExportReportAction = () => {
  ...
  const exportReport = async (id: number, template: File): Promise<ReportExport> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (res.status < 200 || res.status >= 300) {
        ...
        throw new Error(serverMessage ?? FALLBACK_MESSAGE[res.status] ?? "성적서 생성에 실패했습니다.");
      }
      ...
      const message = err instanceof Error ? err.message : "성적서 생성에 실패했습니다.";
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
