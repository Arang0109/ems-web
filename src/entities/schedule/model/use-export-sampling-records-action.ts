...
export const useExportSamplingRecordsAction = () => {
  ...
  const exportSamplingRecords = async (id: number, template: File): Promise<SamplingRecordsExport> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (res.status < 200 || res.status >= 300) {
        ...
        throw new Error(serverMessage ?? FALLBACK_MESSAGE[res.status] ?? "채취기록지 생성에 실패했습니다.");
      }
      ...
      const message = err instanceof Error ? err.message : "채취기록지 생성에 실패했습니다.";
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
  ...
  return { exportSamplingRecords, isLoading, error };
};
