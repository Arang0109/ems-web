...
/**
 * 배출가스 정보(현장측정) 항목 판정 — 측정항목이 THC·NOx·SOx 중 무엇인가.
 *
 * 현장채취 탭은 배정 여부로 입력칸 노출을 정하고, 성적서 탭은 항목별 채취시각을 가져올 때
 * THC(30분)와 가스분석기(15분)를 가른다. 두 화면이 같은 판정을 써야 하므로 여기 둔다.
 *
 ...
 */
export const isExhaustGasPollutant = (
  item: SamplingItemSnapshot, pollutant: ExhaustGasPollutant,
): boolean => (
  item.code
