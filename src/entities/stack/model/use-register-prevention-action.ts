...
export const useRegisterPreventionAction = () => {
  ...
  const registerPrevention = async (stackId: number, data: PreventionCreate) => {
    setIsLoading(true);
    setError(null);
    try {
      const result = await stackApi.registerPrevention(stackId, toRegisterPreventionRequest(data));
      if (!result.status) {
