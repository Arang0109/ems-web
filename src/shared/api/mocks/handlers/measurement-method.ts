...
const mismatch = () => HttpResponse.json(
  { status: false, message: '통칭 시료명은 한 병으로 함께 채취하는 측정방법에만 지정할 수 있습니다.', data: null },
  { status: 400 },
);

const notFound = () => HttpResponse.json(
  { status: false, message: '존재하지 않는 측정방법입니다.', data: null },
  { status: 404 },
);
...
export const measurementMethodHandlers = [
  http.get(`${BASE_URL}/measurement-methods`, () =>
    HttpResponse.json({ status: true, message: '측정방법 목록 조회 성공', data: sorted() })),
  ...
  http.post(`${BASE_URL}/measurement-methods/defaults`, () => {
    ...
    return HttpResponse.json({ status: true, message: '기본 측정방법 채우기 성공', data: sorted() });
  }),

  http.get(`${BASE_URL}/measurement-methods/:methodId`, ({ params }) => {
    const found = findMeasurementMethod(Number(params.methodId));
    if (!found) return notFound();
    return HttpResponse.json({ status: true, message: '측정방법 조회 성공', data: found });
  }),
  ...
  http.post(`${BASE_URL}/measurement-methods`, async ({ request }) => {
    ...
    if (!body.name || !body.sampleGrouping) {
      return HttpResponse.json({ status: false, message: '입력값이 올바르지 않습니다.', data: null }, { status: 400 });
    }
    if (measurementMethods.some((m) => m.name === body.name)) {
      return HttpResponse.json(
        { status: false, message: '이미 같은 이름의 측정방법이 있습니다.', data: null },
        { status: 409 },
      );
    }
    ...
    return HttpResponse.json({ status: true, message: '측정방법 등록 성공', data: created }, { status: 201 });
  }),
  ...
  http.put(`${BASE_URL}/measurement-methods/:methodId`, async ({ params, request }) => {
    ...
    return HttpResponse.json({ status: true, message: '측정방법 수정 성공', data: updated });
  }),
  ...
  http.delete(`${BASE_URL}/measurement-methods/:methodId`, ({ params }) => {
    ...
    if (isReferenced(id)) {
      return HttpResponse.json(
        { status: false, message: '측정물질이 사용 중인 측정방법은 삭제할 수 없습니다.', data: null },
        { status: 409 },
      );
    }
    measurementMethods.splice(index, 1);

    return HttpResponse.json({ status: true, message: '측정방법 삭제 성공', data: null });
  }),
];
