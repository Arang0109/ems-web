  ...
  return (
    <Panel>
      <div className="px-5 pt-5 pb-4 border-b border-rule">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 className="text-h3 text-ink">고객사 목록</h2>
          </div>
          <ProvisionTenantForm
      ...
      <div className="flex items-center justify-start mt-3">
        <Search value={globalFilter} onChange={setGlobalFilter} placeholder={'고객사명, 사업자번호 검색 ...'} />
      </div>
      ...
      {!isLoading && !error && (
        <TableFooterBar table={table} className="border-t border-rule px-5 py-3" />
      )}
    </Panel>
  );
}
